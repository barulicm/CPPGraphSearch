...
std::optional<Path<StateType, ActionType>> BFS(StateType &startState, std::function<bool(const StateType &)> isGoal,
                                       std::function<std::vector<ActionType>(const StateType & )> getAllowedActions,
                                       std::function<StateType(const StateType &, const ActionType &)> getNextState)
{
  ...
  while(!frontier.empty())
  {
    ...
    if( expanded.find(lastState) == expanded.end() ) { // expanded does not contain path's last state
